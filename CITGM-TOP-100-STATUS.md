...
| 67 | whatwg-url | BLOCKED | nested dependency/native addon (browser-reproduced) | Exact native Node CITGM passes, but current-source Chromium (`citgm-1789616765841`) and Firefox (`citgm-1789616802259`) both fail during `pretest` in `webidl2js` → `oxfmt@0.66.0`: `Error: Cannot find native binding` from `oxfmt/dist/bindings-BpewvGlT.js`. oxfmt's published package has native platform optionals but no published `@oxfmt/binding-wasm32-wasi`; its internal fallback therefore has no browser artifact to load. No fake binding or unverified formatter substitute was added. Complete artifacts are preserved under `artifacts/citgm-top-100/rank-067-whatwg-url/`. |
