...
| 88 | form-data | GATE-BLOCKED | ours-side virtual HTTP child lifecycle; published posttest remains unsupported | Exact Node 22.23.2 CITGM for `form-data@4.0.6` at gitHead `64190db548c0179e37206858e39f27cf513e9435` (`native-citgm-node22-current-rerun.log`) reaches all 29 test files (`0 errors in 29 files`) and then fails the package posttest because `npx npm@'>=10.2' audit --production` resolves to `npm@`/`npm@>=10.2: command not found`. Freshly-built Chromium (`postbuild-chromium/citgm-1789667792998`) now reaches the package’s test lifecycle, but the browser runtime throws `ERR_WORKER_EXCEPTION: Callback was already called` from `async` through `vfs.js`; this is the authoritative result for the current source. |
...
assertions pass but the published posttest is unsupported, and the fresh
Chromium run now exposes a generic callback error-boundary defect after the
package test lifecycle begins. The next fix must be validated in Firefox
before rank 88 can be classified.
