...
| 31 | globals | PASS | ours-side npm peer/dependency hoisting | Exact Node 22 CITGM passes on the normal deduped tree. BrowserNpm now schedules ordinary/direct dependency requests ahead of broad peer requests, so a direct `eslint@^8.57.0` constraint establishes the shared root version before a compatible `eslint@>=8.57.0` peer can select `9.x`; a permanent npm-install regression covers the layout. Current Chromium (`citgm-1789692163033`) and Firefox (`citgm-1789692279709`) both pass the complete `globals@17.12.0` CITGM run. Required post-change gates also pass: build:v22, check:wasm, serialized patch regressions, native `npm test`, full Chromium 376/376, and full Firefox 376/376. Complete artifacts and gate logs are under `artifacts/citgm-top-100/rank-031-globals/`. |
...
| 64 | yargs | PASS | ours-side npm lockfile placement plus synchronous worker mailbox | Exact native CITGM (`native-citm-node-rerun.log`) passes `yargs@18.1.0`. Chromium (`citgm-1789687596851`) and Firefox (`citgm-1789687875554`) pass after the installer honors lockfile-nested dependency placement and the worker mailbox fragments large synchronous replies. The permanent lockfile-placement regression is in `tests/npm-install.spec.mjs`; the large worker-reply regression is in `tests/expanded-primitives.spec.mjs`. Required post-change gates pass: serialized patch regressions 4/4; build:v22; check:wasm; native `npm test` 359/359; full Chromium 375/375; full Firefox 375/375. CITGM and gate artifacts are under `artifacts/citgm-top-100/rank-064-yargs/`. |
...
| 67 | whatwg-url | BLOCKED | nested dependency/native addon (browser-reproduced) | Exact native Node CITGM passes. Fresh current-source Chromium (`citgm-1789691488558`) and Firefox (`citgm-1789691585664`) both fail during `pretest` in `webidl2js` → `oxfmt@0.66.0`: `Error: Cannot find native binding`. The published oxfmt package has native platform optionals but no published `@oxfmt/binding-wasm32-wasi`; no fake binding or unverified formatter substitute was added. Complete rerun artifacts are under `artifacts/citgm-top-100/rank-067-whatwg-url/`. |
| 68 | fast-deep-equal | PASS | ours-side browser runtime/VFS layout | Exact native Node CITGM passes. With browser candidate checkouts explicitly placed under the POSIX `/tmp` root (matching native CITGM), legacy TypeScript 3.9 no longer resolves the harness's unrelated root `@types/node` and `undici-types`; Chromium (`citgm-1789663956334`) and Firefox (`citgm-1789664044742`) both pass `fast-deep-equal@3.1.3`. Complete attempt artifacts are preserved under `artifacts/citgm-top-100/rank-068-fast-deep-equal/`. |
...
| 71 | get-stream | BLOCKED | upstream/package assertion (native-reproduced); browser AVA worker does not terminate | Exact native Node 22.23.2 CITGM fails in `test/array-buffer.js`: `buffer.constants.MAX_LENGTH` is `9007199254740991`, so the upstream test computes `chunkCount = 274877906943` and `Array.from({length: chunkCount})` rejects with `RangeError: Invalid array length` before the package implementation is exercised (`native-citgm-node-rerun.log`). Fresh corrected-layout Chromium (`citgm-1789665461314`) and Firefox (`citgm-1789665668220`) both reach the package AVA worker, which remains active without a terminal result; no browser workaround is justified for a native-reproduced upstream assertion. Complete attempted artifacts are preserved under `artifacts/citgm-top-100/rank-071-get-stream/`. |
| 72 | yaml | BLOCKED | upstream/package test-layout failure (native-reproduced); browser harness error also observed | Exact native Node 22.23.2 CITGM reaches 23 passing suites / 1,103 passing tests, then fails because the GitHub archive lacks `tests/yaml-test-suite/src` and `tests/json-test-suite/test_parsing`. Fresh corrected-layout Chromium (`citgm-1789665905890`) and Firefox (`citgm-1789665959608`) both fail in Jest before reaching a contrary package result; Chromium reports `TypeError: require.resolve.paths is not a function`, and Firefox reports the equivalent `findGlobalPaths` failure. Complete artifacts are preserved under `artifacts/citgm-top-100/rank-072-yaml/`. |
...
| Rank 64 `citgm-1789656983966` | With the multi-backed VFS packing attempt, Chromium advanced through install and into the package's c8 phases, then again left `gts`'s nested `eslint` child pending with no terminal transition. | This is a failed runtime-fix attempt, not a pass: the artifact is retained, the multi-backed transport is not accepted as the solution, and rank 64 remains `GATE-BLOCKED`. |
| Rank 64 `citgm-1789657464458` | With the nested CommonJS child kept in the owning gts runtime, Chromium again reaches the package c8 phase; `gts`'s nested `eslint` remains a pending `sync-child` with the owner lifecycle held. | This is another failed runtime-fix attempt, not a pass. The exact browser lifecycle remains ours-side and rank 64 stays `GATE-BLOCKED`. |
| Rank 64 `citgm-1789657697649` | Restoring same-realm execution for children already inside an ESM process does not change the result: Chromium reaches `gts` → `eslint`, where the nested CommonJS child remains pending with `entry-settle` held. | This attempt also failed to unblock the exact browser suite; rank 64 remains `GATE-BLOCKED`. |
...
| Rank 64 `citgm-1789660756903` | Moving callback-based `fs.readdir` completion onto the runtime's poll/microtask path does not change the exact Chromium result: c8 completes, then the real `gts` → `eslint` `esm-child` remains pending with one lifecycle task and no output or state transition; the run was interrupted. | This is a retained failed runtime-fix attempt, not a package pass. The callback-readdir behavior is covered by the permanent nested filesystem regression, but yargs remains `GATE-BLOCKED` until the real nested worker lifecycle completes in both browsers. |
| Rank 64 `citgm-1789661669373` | Deferring the asynchronous nested-worker VFS payload lets Chromium reach the package's c8/mocha path, but the run returns to the real `gts` → `eslint` `esm-child`, which remains pending with one lifecycle task and no terminal output or transition; the run was interrupted. | This is a retained failed runtime-fix attempt. The deferred-payload change is not retained as the solution without a green exact package run; rank 64 remains `GATE-BLOCKED`. |
| Rank 64 `citgm-1789661949827` | Reusing the owning worker's VFS backend for nested tools above 64 MB does not change the exact Chromium result: the package reaches c8/mocha, then the real `gts` → `eslint` `esm-child` remains pending with one lifecycle task and no terminal output or transition; the run was interrupted. | This is a retained failed runtime-fix attempt. The large-VFS same-realm path is not retained without a green exact package run; rank 64 remains `GATE-BLOCKED`. |
| Rank 64 `citgm-1789662168130` | Applying the large-VFS same-realm experiment to nested ESM tools still leaves Chromium at the real `gts` → `eslint` `esm-child` with one pending lifecycle task and no terminal output or transition; the run was interrupted. | This is a retained failed runtime-fix attempt. The experiment is not retained without an exact package pass; rank 64 remains `GATE-BLOCKED`. |
| Rank 64 `citgm-1789672606277` | Fresh Chromium run on the current committed runtime installs and mounts all 158 packages / 4,770 files, completes the package c8 work, then leaves `gts`'s nested `eslint` worker pending with one lifecycle task until the 900-second timeout (`SIGKILL`); no package terminal result is produced. | Confirms the unresolved general nested ESM filesystem-walk lifecycle under the current runtime; no new source or test change is justified by this attempt, and rank 64 remains `GATE-BLOCKED`. |
| Rank 64 `citgm-1789673569796` | Fresh Firefox run installs 157 packages / 4,762 files, completes c8, then leaves the same nested `gts` → `eslint` worker pending until CITGM's npm-test timeout at about 794 seconds; the run exits 1 with `npm-test Timed Out` and no package assertion result. | Cross-browser confirmation of the same general nested ESM filesystem-walk lifecycle blocker; no new source or test change is justified, and rank 64 remains `GATE-BLOCKED`. |
| Rank 64 `citgm-1789677193011` | Chromium run with nested async ESM children forced onto a browser-worker boundary still installs and mounts all 158 packages / 4,770 files, completes c8, then leaves `gts` → `eslint` pending with no child terminal transition; the run was interrupted after the unchanged lifecycle state was confirmed. | Failed runtime-fix attempt; partial telemetry is preserved under `browser-chromium-node22-nested-worker-attempt/citgm-1789677193011/`, and rank 64 remains `GATE-BLOCKED`. |
| Rank 64 `citgm-1789675086246` | Chromium run with the first eager multi-backing transport experiment reaches c8 and then the same nested `gts` → `eslint` child. Generated build files make the snapshot mixed, so the experiment does not cover the 193 MB payload; the run was interrupted after the blocker was reproduced. | Failed runtime-fix attempt; partial CITGM telemetry is preserved under `browser-chromium-node22-eager-pack-attempt/citgm-1789675086246/`, and rank 64 remains `GATE-BLOCKED`. |
| Rank 64 `citgm-1789675584947` | Chromium run with eager packing extended to the mixed 193 MB VFS still reaches c8 and leaves `gts` → `eslint` pending without a child terminal transition; the run was interrupted after the unchanged lifecycle state was confirmed. | Failed runtime-fix attempt; partial telemetry is preserved under `browser-chromium-node22-eager-pack-mixed-attempt/citgm-1789675584947/`, and rank 64 remains `GATE-BLOCKED`. |
| Rank 64 `citgm-1789676205478` | Chromium run with large mixed packing extended to the non-eager initial `gts` boundary still reaches `gts` → `eslint` with no child terminal transition; the run was interrupted after the unchanged lifecycle state was confirmed. | Failed runtime-fix attempt; partial telemetry is preserved under `browser-chromium-node22-eager-pack-mixed2-attempt/citgm-1789676205478/`, and rank 64 remains `GATE-BLOCKED`. |
| Rank 64 `citgm-1789683992045` | Fresh Chromium run on the pre-fragmented-mailbox runtime installs 157 packages / 4,762 files and completes c8/mocha, then the nested `gts` → `eslint` child remains pending until CITGM reports `npm-test Timed Out`; the terminal result exits 1. | Failed runtime-fix attempt; complete telemetry is preserved under `browser-chromium-node22-pre-mailbox-fragment/citgm-1789683992045/`. The large synchronous worker reply is now isolated as the next general runtime fix; rank 64 remains `GATE-BLOCKED`. |
| Rank 64 `citgm-1789686325422` | Chromium with fragmented synchronous replies completes the nested `gts` → `eslint` lifecycle instead of hanging. The exact lint runs to completion but reports 23 `prettier/prettier` errors, so `gts` exits 1 and CITGM reports `The canary is dead`. | The mailbox deadlock is fixed; this run exposes a separate browser formatter/output mismatch. Complete telemetry is preserved under `browser-chromium-node22-mailbox-fragment/citgm-1789686325422/`, and rank 64 remains `GATE-BLOCKED` pending native-vs-browser comparison. |
...
| Rank 67 `citgm-1789616765841` / `citgm-1789616802259` | `whatwg-url` reaches `pretest`, then `scripts/transform.js` imports `webidl2js@21.0.0`, whose `oxfmt@0.66.0` dependency throws `Error: Cannot find native binding` in both browsers. The exact native Node run passes. | The blocker is a nested toolchain artifact boundary, not the candidate package: oxfmt embeds a legitimate WASI lookup path but publishes neither a WASI binding package nor a WASI binary, while its native binding packages cannot execute in a browser. A custom formatter or fake binding would change the tested toolchain, so rank 67 remains `BLOCKED` pending an upstream/browser-compatible oxfmt artifact. |
| Rank 68 `citgm-1789616890072` / `citgm-1789616955258` | The original browser runs used a candidate checkout nested below `/node`; TypeScript 3.9 therefore walked into the harness's modern root `@types/node` and `undici-types` declarations and reported syntax errors. | Reproduced and fixed without a package shim by running CITGM with `--tmpDir /tmp`, which gives the virtual checkout the same ancestor layout as native CITGM. Chromium `citgm-1789663956334` and Firefox `citgm-1789664044742` pass the full package suite; rank 68 is promoted to `PASS`. |
...
| Rank 71 `native-citgm-node-rerun.log` / `citgm-1789665461314` / `citgm-1789665668220` | Node 22.23.2 fails the upstream oversized-buffer test because its `MAX_LENGTH` value makes the test’s `Array.from` length invalid. Fresh corrected-layout Chromium and Firefox both reach AVA but remain active without a terminal package result; the saved progress shows no contrary implementation failure. | Native-reproduced package assertion blocker. The package test must be corrected upstream for Node 22; changing browser constants or injecting a package-specific test shim would invalidate the CITGM oracle. Recorded `BLOCKED`. |
...
| `citgm-1789665905890` / `citgm-1789665959608` | Fresh corrected-layout Chromium and Firefox fail in Jest’s browser-incompatible `require.resolve.paths` path before reaching the missing fixture assertions; the native run remains the authoritative package/source-archive failure. | Supplying omitted fixtures or adding a package-specific Jest resolver shim would change the upstream oracle; no such workaround was added. Recorded `BLOCKED`. |
...
| `brand-chromium/citgm-1789666723376` | After adding a cross-child BrowserAgent brand and allowing local dispatch before proxy selection, Chromium reaches both POST requests but still times out at the `node`/`istanbul` completion boundary; both requests are recorded as raw-socket requests. | The attempt disproves `instanceof` as the only cause but does not prove a runtime fix. The complete artifact is retained for the next investigation. |
| `default-agent-chromium/citgm-1789666868384` | Marking requests that omitted `agent` as default-agent requests does not change the result: Chromium still records both POSTs through raw sockets and times out at the same inner lifecycle boundary. | The remaining bypass is the configured proxy capability guard, which is being removed for local virtual bindings. |
| `local-dispatch-chromium/citgm-1789667007757` | With local virtual dispatch ahead of configured proxy selection, the previous raw-socket timeout changes to `ERR_WORKER_EXCEPTION: Callback was already called` from the upstream `async` lifecycle. | This proves the local request now reaches the virtual network; the remaining failure is a streamed HTTP lifecycle regression. |
| `local-dispatch-firefox/citgm-1789667191582` | Firefox reproduces the same `ERR_WORKER_EXCEPTION`, with a full stack through `async/dist/async.js` (`onlyOnce`/`_asyncMap`) and VFS `asyncFsOperation`; it does not return to the earlier multipart assertion. | Cross-browser evidence narrows the remaining defect to duplicate callback completion in the VFS realpath/file-matching lifecycle. |
| `realpath-chromium/citgm-1789667527990` | Moving VFS `fs.realpath` to poll-like microtask completion leaves the exact Chromium failure unchanged: Istanbul still reports `Callback was already called`. | The focused callback oracle remains green, but the candidate’s file walk exposes a narrower path/error case that still needs tracing. |
| `realpath-diagnostic-chromium/citgm-1789667661026` | A temporary VFS completion probe records only two host `/node/node_modules/.bin/npm` realpath calls; no candidate `/tmp/.../form-data` paths reach the modified source, while the stale bundle still reports `Callback was already called`. | The exact run was not authoritative for the source fix. The probe is removed and a fresh Node 22 runtime build is required before rerunning form-data. |
| `postbuild-chromium/citgm-1789667792998` | After `npm run build:v22`, Chromium installs the exact `4.0.6` candidate, runs the complete lint phase, and then fails at the package test lifecycle with `ERR_WORKER_EXCEPTION: Callback was already called`. The stack enters `async/dist/async.js` from `runtime/vfs.js` while a callback exception is being re-reported. | Authoritative current-bundle evidence. The remaining defect is in the generic asynchronous filesystem callback error boundary, not stale generated output; a focused regression is being added before the Firefox rerun. |
...
## Latest browser-unblock continuation (Node 22.23.2)

This continuation supersedes the earlier rank-68 `BLOCKED` entry. The exact
native run still passes, and the final browser CITGM runs are preserved under
`artifacts/citgm-top-100/rank-068-fast-deep-equal/`:

| Run / log | Observed result | Classification and resolution |
| --- | --- | --- |
| `fix4-chromium/citgm-1789635856243` | `fast-deep-equal@3.1.3` completes its browser CITGM lifecycle with exit 0; child output is present and terminal failure excerpts are empty. | PASS in Chromium after general fixes for candidate temp-root visibility, install-prefix VFS mounting, non-configurable process-bound descriptors, worker `process.reallyExit`, and nested multi-backing shared VFS transport. |
| `fix4-firefox/citgm-1789635859607` | The same package lifecycle completes with exit 0; terminal failure excerpts are empty. | PASS in Firefox with Chromium parity; no package-specific shim was added. |
| `process-worker.spec.mjs` Chromium / Firefox | The focused worker-process suite passes 23/23 in both browsers, including the permanent `signal-exit` process-contract regression and the large multi-backing VFS regression. | Permanent browser regressions are green. |
| `patch-regressions.mjs` / `build:v22` / `check:wasm` | Serialized runtime regressions pass; the Node 22 build produces five WASM artifacts and all five pass export validation. | Required pre-CITGM gates are green. |
| full Node 22 `npm test` / full Chromium / full Firefox | Native tests pass 359/359; Chromium passes 367/367; Firefox passes 367/367. | All required post-source-change repository gates are green. |

Rank 68 is promoted to `PASS`. The first remaining ours-side browser blocker in
rank order is rank 64 `yargs`; rank 67 `whatwg-url` remains a nested native-addon
blocker, and rank 93 `ajv` remains an upstream fixture/submodule blocker. The
fresh rank-93 native, Chromium, and Firefox rerun artifacts are committed with
this checkpoint.
