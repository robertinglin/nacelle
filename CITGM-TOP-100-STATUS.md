...
| 88 | form-data | GATE-BLOCKED | ours-side virtual HTTP child lifecycle; published posttest remains unsupported | Exact Node 22.23.2 CITGM for `form-data@4.0.6` at gitHead `64190db548c0179e37206858e39f27cf513e9435` (`native-citgm-node22-current-rerun.log`) reaches all 29 test files (`0 errors in 29 files`) and then fails the package posttest because `npx npm@'>=10.2' audit --production` resolves to `npm@`/`npm@>=10.2: command not found`. Current Chromium (`citgm-1789666723376`, after the local-dispatch brand change) reaches the package's two POST requests but still times out at the inner `node`/`istanbul` lifecycle boundary after 5000 ms; the runtime records both requests through the raw-socket path. Earlier Chromium/Firefox retries and current Firefox evidence remain the same field-count/child-lifecycle failures. The current source still has an ours-side virtual HTTP child regression to fix before promotion. |
