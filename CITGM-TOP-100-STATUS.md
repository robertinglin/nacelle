...
| 88 | form-data | GATE-BLOCKED | ours-side virtual HTTP child lifecycle; published posttest remains unsupported | Exact Node 22.23.2 CITGM for `form-data@4.0.6` at gitHead `64190db548c0179e37206858e39f27cf513e9435` (`native-citgm-node22-current-rerun.log`) reaches all 29 test files (`0 errors in 29 files`) and then fails the package posttest because `npx npm@'>=10.2' audit --production` resolves to `npm@`/`npm@>=10.2: command not found`. Current Chromium (`citgm-1789667007757`) and Firefox (`citgm-1789667191582`) both use the local virtual-dispatch path but fail with upstream `ERR_WORKER_EXCEPTION: Callback was already called`; Firefox’s stack identifies the duplicate callback at `async/dist/async.js` while Istanbul calls VFS `fs.realpath`. The current source still has an ours-side virtual HTTP/VFS lifecycle regression to fix before promotion. |
