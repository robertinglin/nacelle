...
| 64 | yargs | GATE-BLOCKED | ours-side npm lockfile placement plus synchronous worker mailbox; Firefox and post-change gates pending | Exact native CITGM (`native-citm-node-rerun.log`) passes `yargs@18.1.0`. Chromium (`citgm-1789687596851`) now passes after the installer honors lockfile-nested dependency placement and the worker mailbox fragments large synchronous replies. The permanent lockfile-placement regression is in `tests/npm-install.spec.mjs`; the large worker-reply regression is in `tests/expanded-primitives.spec.mjs`. Exact Firefox and required post-change repository gates remain pending before rank promotion. |
