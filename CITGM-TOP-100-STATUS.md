...
| 68 | fast-deep-equal | PASS | ours-side browser runtime/VFS layout | Exact native Node CITGM passes. After placing browser candidate checkouts under the POSIX `/tmp` root, mounting installs at their actual `node_modules` prefix, preserving non-configurable process-bound descriptors, exposing `process.reallyExit` in worker globals, and retaining multi-backing shared VFS views across nested workers, current-source Chromium (`citgm-1789635856243`) and Firefox (`citgm-1789635859607`) both pass `fast-deep-equal@3.1.3`. Required post-change gates are green; complete attempt artifacts are preserved under `artifacts/citgm-top-100/rank-068-fast-deep-equal/`. |
