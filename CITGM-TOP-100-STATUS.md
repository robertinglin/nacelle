...
| 64 | yargs | GATE-BLOCKED | ours-side nested worker/runtime lifecycle (browser failure remains) | Exact native CITGM (`native-citgm-node-rerun.log`) passes `yargs@18.1.0`. Chromium (`citgm-1789656983966`) still reaches the nested `gts` → `eslint` child with the packed VFS and then remains pending; the attempt was interrupted after the child state stopped changing. The retained source/test changes and full Node 22 gates are green, but the exact browser CITGM lifecycle is not; the rank is not advanced. |
