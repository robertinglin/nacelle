...
test.describe('browser runtime bridge and core primitives', () => {
    ...
    `, {
      ...
      files: {
        ...
        '/node/node_modules/.bin/eslint': [
          ...
          "process.on('uncaughtException', error => { console.error(error); process.exitCode = 2; });",
          "process.on('unhandledRejection', error => { console.error(error); process.exitCode = 2; });",
          'async function run() {',
          '  const readdir = directory => new Promise((resolve, reject) => {',
          "    fs.readdir(directory, { withFileTypes: true }, (error, entries) => error ? reject(error) : resolve(entries));",
          '  });',
          '  const stat = path => new Promise((resolve, reject) => {',
          "    fs.stat(path, (error, value) => error ? reject(error) : resolve(value));",
          '  });',
          '  const walkStream = directory => {',
          '    let destroyed = false;',
          ...
          '            else { await stat(entryPath); if (!stream.push({ path: entryPath })) return; }',
          '          }',
          '        }',
          ...
          "    const stream = walkStream('/node/tree');",
          '    stream.on(\'data\', () => { stream.destroy(); resolve(); });',
        ...
        ...Object.fromEntries(Array.from({ length: 5000 }, (_, index) => [
          `/node/tree/dir-${Math.floor(index / 50)}/file-${index % 50}.js`,
          `module.exports = ${index};`,
        ])),
        '/node/node_modules/execa/package.json': JSON.stringify({ main: 'index.js' }),
        '/node/node_modules/execa/index.js': [
  ...
  test('preserves async function expression calls while transforming awaits', async ({ harnessPage }) => {
    const result = await harnessPage.run(`
      (async () => {
        const value = await function ({ answer }) {
          return answer;
        }({ answer: 42 });
