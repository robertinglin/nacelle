...
class JsonlWorker:
    ...
    @staticmethod
    def _read_stderr(process: subprocess.Popen[str], output: deque[str]) -> None:
        assert process.stderr is not None
        try:
    ...
    def call(self, request: dict[str, Any], timeout_seconds: float) -> tuple[dict[str, Any] | None, str, int, bool]:
...
class JsonlPool:
    ...
    def call(self, request: dict[str, Any], timeout_seconds: float) -> tuple[dict[str, Any] | None, str, int, bool]:
        ...
        try:
            return worker.call(request, timeout_seconds)
        finally:
            self.available.put(worker)
