...
def surface_probe_source(modules: Sequence[str]) -> str:
    ...
    return (
        ...
        "          && (prototypeOfValue === Object.prototype || prototypeOfValue === null);\n"
        "        const inspectOwnObject = typeof value === \"function\" || (\n"
        "          isPlainObject && !key.startsWith(\"_\") &&\n"
        "          (stableObjectKeys.has(key) || Object.getOwnPropertyNames(value).some((name) => {\n"
        "            try { return typeof value[name] === \"function\"; } catch { return false; }\n"
        "          }))\n"
        "        );\n"
        "        const ownNames = typeof value === \"function\"\n"
        "          ? Object.getOwnPropertyNames(value).filter((name) => ![\"length\", \"name\", \"prototype\", \"arguments\", \"caller\"].includes(name))\n"
        "          : inspectOwnObject ? Object.getOwnPropertyNames(value).filter((name) => !/^\\d+$/.test(name)) : [];\n"
        "        for (const member of [\n"
        "          ...ownNames,\n"
        ...
        "          }\n"
        "        }\n"
        "        let prototype = typeof value === \"function\" ? value.prototype : prototypeOfValue;\n"
